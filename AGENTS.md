...
- Extended-stable is a single N-1 line: the trailing completed month relative to `main`'s version. Older `.33+` lines retire when `main` advances another month; publishing a retired line requires an explicit maintainer decision, not a routine guard bypass.
