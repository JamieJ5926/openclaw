...
Open **Settings → Models** in the Control UI to add, replace, or remove provider API keys. Key edits use the same core credential writer as `models auth paste-api-key`; configured connections reference the saved profile instead of keeping another copy of the key. Removing API keys clears inline keys, saved API-key profiles, and their config bindings. Other sign-in methods, environment-provided keys, and secret references remain unchanged. The page shows credential sources without displaying secrets.
