...
- Configured catalogs resolve provider aliases once when they are built. Loading config preserves authored provider model IDs, so catalog preparation and compaction can resolve them consistently. Selecting one of the resulting model IDs preserves it when configuration is saved and loaded again. Retired-model migrations still apply.
- Catalog defaults fill omitted metadata from the selected model. Loading config does not turn those defaults into authored overrides of another model or route.
