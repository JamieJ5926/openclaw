...
Bare configured aliases resolve to their canonical provider and model before compaction starts. If a bare value matches both an alias and a configured literal model ID, the literal model ID wins. Literal model IDs are case-sensitive; configured aliases are case-insensitive. An unmatched bare value remains a model ID on the active provider.
