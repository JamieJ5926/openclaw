...
`contracts.accountUsageProviders` optionally lists the subset of those provider ids that support exact OAuth/token profile authentication and final-I/O ownership checks. Without it, usage remains provider-wide and is not attributed to a saved login. See the [provider usage hook contract](/plugins/sdk-provider-plugins).
