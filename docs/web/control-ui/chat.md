...
On wide desktop panes, a rail of dots sits in the transcript's right gutter. It shows up to ten evenly sampled message landmarks, including both ends of the conversation. The track is half its former height: up to 210px, giving five dots 42px center-to-center spacing and ten dots 21px spacing. Dots retain their 9px size; hit areas fit between adjacent landmarks so they do not overlap. The current landmark is accented. Hover for a short Markdown message preview, or click a dot to jump to that message. Hovering also grows nearby ticks; the preview stays open when you move onto it. Tab focuses the rail; arrow keys move between landmarks, Enter or Space jumps, Home and End select the endpoints, and Escape dismisses the preview. A focused landmark stays focused when messages arrive. An empty message preview shows “Preview unavailable.” The rail stays hidden on mobile, in narrow or short panes, and when your saved message width leaves too little right-side gutter space. A jump briefly tints the target message with a soft background, fading over 1.2 seconds without a border or ring. Reduced motion disables dot and tick transitions and shows the target tint statically for one second.
