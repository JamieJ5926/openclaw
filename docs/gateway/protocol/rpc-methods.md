...
<AccordionGroup>
  ...
  <Accordion title="Plugin management">
    ...
    - `plugins.catalog.browse` (`operator.read`) returns normalized ClawHub code-plugin and bundle-plugin entries joined with Gateway-owned local state. Pass optional `query`, `intent` (`all`, `trending`, or `official`), category slug, opaque `cursor`, and `pageSize` from 1 to 100. Search requests do not accept a browse cursor. Results separate remote `catalog` facts from local `present`, `installed`, `enabled`, `state`, and eligible `action` facts; `nextCursor`, when present, must be returned unchanged on the next browse request.
    - `plugins.catalog.categories` (`operator.read`) returns ClawHub's canonical category order as bounded slug, label, description, and validated icon-key records.
    - `plugins.catalog.get` (`operator.read`) accepts an opaque discovery `id` returned by browse and returns that normalized catalog entry joined with current Gateway local state. Discovery failures return retryable `UNAVAILABLE`; clients should keep the installed inventory usable and offer retry rather than treating ClawHub as local runtime state.
