...
<AccordionGroup>
  ...
  <Accordion title="Plugin management">
    ...
    - `plugins.search` (`operator.read`) searches installable ClawHub code-plugin and bundle-plugin families. Pass non-empty `query` and optional `limit` from 1 to 100. The Control UI adds optional `searchSource: "openclaw-control-ui"` only for manual input after its 300 ms debounce (minimum two characters) or a deliberate ClawHub-search action. Automatic refreshes, CLI searches, and generic requests omit it. The Gateway also omits the outgoing marker when the existing `CLAWHUB_DISABLE_TELEMETRY` opt-out is enabled.
    - Search attribution is not authorization: existing ClawHub access remains unchanged. A marked request uses one combined ClawHub response and is not automatically retried. ClawHub records bounded search facts after completion, without adding user, device, session, or version fields. Canceling before dispatch records nothing; a late browser cancellation cannot retract a search already completed by ClawHub.
