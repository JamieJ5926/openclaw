...
import { CHANNEL_INGRESS_OBSERVABILITY_METADATA_KEY } from "./ingress-observability.js";
import {
  createChannelIngressQueue,
  getChannelIngressDiagnosticSnapshot,
...
describe("channel ingress queue", () => {
  ...
  it("records claim-fenced progress and reports active operations from live owners only", async () => {
    await withTempState(async (stateDir) => {
      ...
      openIngressStateDatabase(stateDir).db
        .prepare(
  ...
  it("preserves non-object provider metadata byte-for-byte as unknown ingress progress", async () => {
    await withTempState(async (stateDir) => {
      ...
      await primitiveQueue.enqueue("primitive", { text: "primitive" }, {
        metadata: "provider-token",
        receivedAt: 10,
      });
      await arrayQueue.enqueue("array", { text: "array" }, {
  ...
  it("preserves provider-owned ingressProgress collisions without changing claim behavior", async () => {
    await withTempState(async (stateDir) => {
      const queue = createTestIngressQueue<{ text: string }, Record<string, unknown>, Record<string, unknown>>(
        stateDir,
      );
      ...
      await queue.enqueue("object-collision", { text: "fail" }, { metadata: objectCollision, receivedAt: 10 });
      await queue.enqueue("string-collision", { text: "complete" }, { metadata: stringCollision, receivedAt: 20 });
      await queue.enqueue("schema-shaped", { text: "pending" }, { metadata: schemaShapedCollision, receivedAt: 30 });
      ...
      await expect(
        queue.updateProgress?.(objectClaim, { stage: "thread_history", observedAt: 40, progressAt: 40 }),
      ).resolves.toBe(false);
      await expect(
        queue.updateProgress?.(stringClaim, { stage: "thread_history", observedAt: 45, progressAt: 45 }),
      ).resolves.toBe(false);
      await expect(queue.fail(objectClaim, { reason: "poison", message: "bad", failedAt: 50 })).resolves.toBe(
        true,
      );
      await expect(
  ...
  it("preserves progress across automatic retry without resetting no-progress age", async () => {
    await withTempState(async (stateDir) => {
      ...
      await expect(queue.release(firstClaim, { lastError: "retry", releasedAt: 120 })).resolves.toBe(
        true,
      );
