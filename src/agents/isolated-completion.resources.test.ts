...
it.each([
  ["standalone", "overlap"],
  ["standalone", "callback-tail"],
  ["standalone", "cancel-tail"],
  ["standalone", "auth-tail"],
  ["standalone", "auth-failure"],
  ["borrowed", "callback-tail"],
  ["borrowed", "auth-tail"],
] as const)("retains %s isolated completion resources through %s", async (owner, mode) => {
  ...
  let firstResourceRead: (() => number) | undefined;
  let responseWorkFinished = false;
  ...
  Object.defineProperty(globalThis, globalKey, {
    ...
    value: {
      connections,
      prepare: (readResource: () => number) => {
        if (++authCalls !== 1) {
          return;
        }
        firstResourceRead = readResource;
  ...
  fs.writeFileSync(
    ...
    `module.exports = { id: ${JSON.stringify(fixture.pluginId)}, register(api) {
        ...
        globalThis[${JSON.stringify(globalKey)}].connections.push(db);
        db.exec("CREATE TABLE answer(value INTEGER); INSERT INTO answer VALUES (42)");
        const read = () => db.prepare("SELECT value FROM answer").get().value;
        api.lifecycle.registerRuntimeLifecycle({ id: "isolated-database", dispose() {
          db.close();
        } });
        api.registerProvider({
          id: ${JSON.stringify(fixture.providerId)}, label: "Isolated resources", auth: [],
          async prepareRuntimeAuth() { globalThis[${JSON.stringify(globalKey)}].prepare(read); }
        });
      } };`,
  );
  ...
  try {
    ...
    await withEnvAsync(
      ...
      async () => {
              ...
              return (model, context, options) =>
                stream(model, context, {
                  ...
                  onResponse: async (response, responseModel) => {
                    ...
                    actualWork.push(pending);
                    await pending;
                    expect(firstResourceRead?.()).toBe(42);
                    responseWorkFinished = true;
                  },
                });
            }),
          );
        ...
        const start = (signal?: AbortSignal, borrow = false) => {
          const request = {
            config: cfg,
          ...
          const pending = borrow
            ? withPreparedModelSelection(
                { cfg, agentId: "main", readOnly: true },
                [{ provider: fixture.providerId, modelId: "isolated-model", agentId: "main" }],
                async (selection) =>
                  await runIsolatedCompletion({
                    ...request,
                    config: selection.preparedModelRuntime.config,
                    agentDir: selection.preparedModelRuntime.agentDir,
                    workspaceDir: selection.workspaceDir,
                    assertCurrent: selection.assertCurrent,
                    borrowPreparedRuntime: selection.borrowPreparedRuntime,
                  }),
              )
        ...
        const first = start(
          mode === "callback-tail" ? controller.signal : undefined,
          owner === "borrowed",
        );
        if (mode === "auth-failure") {
        ...
        if (owner === "borrowed") {
          const closing = closeEphemeralPreparedModelRuntimeResources();
          try {
            expect(firstResourceRead?.()).toBe(42);
          } finally {
            finishWork.resolve();
          ...
          await closing;
          expect(firstResourceRead).toThrow();
          if (mode === "auth-tail") {
            expect(authWorkFinished).toBe(true);
          } else {
            expect(responseWorkFinished).toBe(true);
          }
          return;
        }
