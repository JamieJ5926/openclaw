...
describe("runIsolatedCompletion", () => {
  it.each([
    ...
    ["denied", "main", undefined],
    ["success", "worker", undefined],
    ["success", "worker", "worker"],
    ["mismatch", "worker", "main"],
  ] as const)(
    "releases one selected owner after %s (owner=%s, requested=%s)",
    async (mode, agentId, requestedAgentId) => {
      ...
      Object.assign(preparedModelRuntime, { agentId });
      const operation = withPreparedModelSelection({ cfg: {}, agentId }, [], async (context) => {
