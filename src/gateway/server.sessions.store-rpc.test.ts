...
defineSessionGroupRpcTests({ createSessionStoreDir, openClient });
...
test("write-scoped operators manage chat organization but not admin session settings", async () => {
  ...
  try {
    ...
    const pinned = await rpcReq<{ entry: { pinnedAt?: number } }>(ws, "sessions.patch", {
      key: "agent:main:topic-a",
      pinned: true,
    });
    expect(pinned.ok).toBe(true);
    ...
    const organized = await rpcReq<{ entry: { category?: string } }>(ws, "sessions.patch", {
      key: "agent:main:topic-a",
      category: "Travel",
      unread: true,
    });
    ...
    const archived = await rpcReq<{ entry: { archivedAt?: number } }>(ws, "sessions.patch", {
