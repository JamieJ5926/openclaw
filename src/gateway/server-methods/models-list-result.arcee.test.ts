...
it("keeps exact authored identities ahead of provider-owned wire aliases", () => {
  ...
  expect(
    findProviderModelConfig(models, "arcee", "trinity-large-thinking", normalize),
  ).toMatchObject({ name: "Exact row", contextWindow: 65536 });
  expect(
    findProviderModelConfig([models[0]!], "arcee", "trinity-large-thinking", normalize),
  ).toMatchObject({ name: "Wire row", contextWindow: 32768 });
  expect(
    findProviderModelConfig(models, "arcee", "openrouter/trinity-large-thinking", normalize),
  ).toBeUndefined();
});
...
it("keeps unknown model case and foreign provider prefixes distinct in authored lookup", () => {
  ...
  expect(findProviderModelConfig(models, "custom", "Exact", normalize)).toMatchObject({
    name: "Case sensitive",
  });
  expect(findProviderModelConfig(models, "custom", "exact", normalize)).toBeUndefined();
  expect(findProviderModelConfig(models, "custom", "other/Exact", normalize)).toBeUndefined();
});
