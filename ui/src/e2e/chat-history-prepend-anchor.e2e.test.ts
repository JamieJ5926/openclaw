...
type AnchorFrames = { frame: number; positions: Array<number | null> };
type AnchorWindow = Window & { prependFrames: AnchorFrames };
...
suite.define(() => {
  it.each([
    ...
    "anchors history prepend (shared=$sharedGroup, manual=$manual, onlyGroup=$onlyGroup, more=$more, persisted=$persisted)",
    async ({ sharedGroup, manual, onlyGroup, more, persisted = true }) => {
      ...
      try {
        ...
        await page.evaluate(
          (messageKey) => {
            const frames: AnchorFrames = { frame: 0, positions: [] };
            ...
            const sample = () => {
              ...
              frames.positions.push(bubble?.getBoundingClientRect().top ?? null);
