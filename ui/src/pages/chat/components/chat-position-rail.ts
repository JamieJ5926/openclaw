...
class ChatPositionRailDirective extends AsyncDirective {
  ...
  render({
    ...
    // Keep the original bounded landmarks and both conversation endpoints.
    const markers = indexes.map((candidateIndex, index) => {
      const candidate = candidates[candidateIndex]!;
      const role = resolveMessageRole(candidate.message);
      return {
        id: candidate.id,
        label: t(
          role === "user"
    ...
    // Preserve current sanitized Markdown previews; parse only the open preview.
    ...
    return html`
      ...
      >
        <div
          ...
          style=${styleMap({ "--chat-position-marker-count": String(count) })}
        >
          ${repeat(
            markers,
            (marker) => marker.id,
            (marker, index) => {
              ...
              return html`
                <button
                  ...
                  aria-description=${marker.id === previewMarker?.id && previewText ? `${previewText}. ${t("chat.thread.positionMarkerHint")}` : t("chat.thread.positionMarkerHint")}
            ...
            previewMarker
              ? html`
                  ...
                  >
                    ...
                    <!-- Preview links remain inert; the marker owns navigation. -->
