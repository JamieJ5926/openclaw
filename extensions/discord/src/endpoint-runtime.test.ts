...
function installLoopbackEndpoint(baseUrl: string): DiscordEndpointLease {
  lease = installDiscordEndpointRuntime({
    restApiBaseUrl: `${baseUrl}/api/v10`,
    gatewayBotUrl: `${baseUrl}/api/v10/gateway/bot`,
    gatewayOrigin: `ws://${new URL(baseUrl).host}`,
  });
  return lease;
}

describe("Discord endpoint runtime", () => {
  it("is absent by default and restores the live RequestClient behavior when closed", async () => {
    expect(getDiscordEndpointRuntime()).toBeUndefined();
    const endpoint = installLoopbackEndpoint("http://127.0.0.1:43210");
    expect(getDiscordEndpointRuntime()).toBe(endpoint.runtime);
    endpoint.close();
    lease = undefined;
  ...
  it("routes REST requests with their authentication and body to the configured origin", async () => {
    ...
    installLoopbackEndpoint(baseUrl);
  ...
  it("invalidates retained clients before I/O when a lease closes or is replaced", async () => {
    const observedUrls: string[] = [];
    const baseUrl = await startEndpointServer((request) => {
      observedUrls.push(request.url ?? "");
    });
    const retiredLease = installLoopbackEndpoint(baseUrl);
    const retainedClient = new RequestClient("must-not-leak", { queueRequests: false });

    retiredLease.close();
    installLoopbackEndpoint(baseUrl);

    await expect(retainedClient.get("/users/@me")).rejects.toThrow(/lease has been retired/);
  ...
  it("rejects public REST, CDN media, and upload targets before network I/O", async () => {
    ...
    await expect(
      endpoint.runtime.fetch("https://discord.com/api/v10/users/@me", {
  ...
  it("accepts only the configured Gateway origin for initial and resume sockets", () => {
    installLoopbackEndpoint("http://127.0.0.1:43210");
  ...
  it("rejects non-loopback plaintext and private WSS descriptors", () => {
    expect(() =>
      installDiscordEndpointRuntime({
        restApiBaseUrl: "http://example.com/api/v10",
        gatewayBotUrl: "http://example.com/api/v10/gateway/bot",
        gatewayOrigin: "ws://example.com",
      }),
    ).toThrow(/HTTPS or loopback HTTP/);
    expect(() =>
      installDiscordEndpointRuntime({
        restApiBaseUrl: "https://example.com/api/v10",
        gatewayBotUrl: "https://example.com/api/v10/gateway/bot",
        gatewayOrigin: "wss://127.0.0.1",
      }),
    ).toThrow(/private\/internal\/special-use/);
  });
});
