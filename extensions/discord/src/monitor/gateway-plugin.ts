...
import {
  ...
  assertDiscordEndpointRuntimeActive,
...
function createGatewayPlugin(params: {
  ...
  class OpenClawGatewayPlugin extends discordGateway.GatewayPlugin {
    ...
    constructor() {
      ...
      params.endpoint?.runtime.signal.addEventListener(
        "abort",
        () => {
          const socket = this.ws;
          this.disconnect();
          const terminate = socket ? Reflect.get(socket, "terminate") : undefined;
          if (socket && typeof terminate === "function") {
            try {
              Reflect.apply(terminate, socket, []);
            } catch {
              // Best-effort teardown after revocation.
            }
          }
        },
        { once: true },
    ...
    override createWebSocket(url: string) {
      ...
      if (params.endpoint) {
        assertDiscordEndpointRuntimeActive(params.endpoint.runtime);
      }
